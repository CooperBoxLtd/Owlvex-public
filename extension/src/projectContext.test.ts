...
describe('project root helpers', () => {
    ...
    it('loads the configured TDD Box file by default and skips it when explicitly disabled', async () => {
        ...
        expect(disabledContext.summary).toContain('TDD file docs\\app.tdd.md');
        expect(disabledContext.combined).toContain('TDD: preserve Morse packet format.');
        ...
        const explicitlyDisabledContext = await loadProjectContextInfo({
            targetUris: [vscode.Uri.file('D:\\repo\\tools\\benchmark-app\\src\\server.js')],
        });
        expect(explicitlyDisabledContext.summary).not.toContain('TDD file');
