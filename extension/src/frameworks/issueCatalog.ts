...
const ISSUE_DEFINITIONS: CanonicalIssueDefinition[] = [
    ...
    {
        id: 'owlvex.issue.client_controlled_identity_headers.001',
        slug: 'client-controlled-identity-or-role-headers',
        title: 'Client-controlled identity or role headers trusted as authentication',
        category: 'authentication',
        family: 'family.identity_auth',
        severity: 'CRITICAL',
        detectionLevel: 'direct',
        ...
        keywords: [
            ...
            'req.headers',
            'attachsession',
            'session built from headers',
            'header-based session',
            'client-controlled role',
        ],
        negativeKeywords: ['jwt.verify', 'signature verification', 'trusted upstream proxy', 'strip external identity headers'],
        minimumScore: 35,
        remediationSummary: 'Derive identity and roles from a verified server-side authentication source such as a validated JWT, signed session, or trusted internal gateway; never trust raw client-supplied identity or role headers.',
        cheatSheetRefs: ['OWASP Authentication Cheat Sheet', 'OWASP Authorization Cheat Sheet'],
    },
    {
