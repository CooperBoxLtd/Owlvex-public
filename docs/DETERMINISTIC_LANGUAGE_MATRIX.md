...
| --- | --- | --- | --- | --- | --- | --- |
| Command injection | Yes | Yes | Yes | Yes | Yes | All five language cells now emit Engine 1.0 evidence contracts in the regression gate: command text or request-derived command variable, process execution sink, missing fixed-executable/non-shell argument guard, and safe companion suppression. Python covers `os.system(...)` and `subprocess.*(..., shell=True)`; Java covers `Runtime.getRuntime().exec(...)`; C# covers `Process.Start(...)`; Go covers `exec.Command("sh", "-c", ...)` on request-derived command strings |
| SQL injection | Yes | Yes | Yes | Yes | Yes | All five language cells now emit Engine 1.0 evidence contracts in the regression gate: dynamic SQL text, execution sink, missing parameter-binding guard, and safe parameterized companion suppression. Python covers f-string SQL; Java covers concatenated SQL reaching `Statement.execute*`; C# covers concatenated SQL reaching `SqlCommand`; Go covers concatenated SQL reaching `db.Query/Exec/QueryRow` |
...
| SSRF | Yes | Yes | Yes | Yes | Yes | All five language cells now emit Engine 1.0 evidence contracts in the regression and proof-contract benchmark gates: request-controlled outbound destination, outbound network sink, missing trusted-destination allowlist guard, and safe companion suppression. JavaScript also flags weak substring host checks; Python covers direct `requests.*(...)`; Java covers `new URL(requestInput).openStream()`; C# covers `HttpClient.GetStringAsync(...)`; Go covers `http.Get(...)` / `client.Get(...)` on request-derived destinations |
