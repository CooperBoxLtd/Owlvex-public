...
**Diagram Box** - local Mermaid diagram workflow for project understanding. The primary outputs are Architecture Map, Threat Flow Diagram, and post-scan Risk Lens. The Security Evidence Map remains available as an advanced scanner-grounded traceability layer. Workflow and TDD Diff diagrams are deferred until they can be generated from stronger project-specific and requirement-level evidence. Diagrams must distinguish confirmed relationships from inferred relationships.
