...
**Diagram Box** - local Mermaid diagram workflow for project understanding. The primary outputs are Architecture Map, Threat Flow Diagram, and post-scan Risk Lens. The Architecture Map should explain runtime responsibilities and flow first, using concrete files as evidence labels rather than showing a raw import graph as the main story. Architecture labels must stay domain-neutral and portable across apps. The Security Evidence Map remains available as an advanced scanner-grounded traceability layer and is the proof layer for imports, guards, sinks, stores, and integrations. Workflow and TDD Diff diagrams are deferred until they can be generated from stronger project-specific and requirement-level evidence. Diagrams must distinguish confirmed relationships from inferred relationships.
